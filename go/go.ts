...
export async function PlayAGame(ns: NS) {
  ...
  do {
    ...
    const chosen_move = captureMove ?? weakenMove ?? expansion_move ?? random_move;
  ...
  for (const chain of chains) {
    if (liberties_board[chain.points[0].x][chain.points[0].y] > 0) {
      for (const point of chain.points) {
        ...
        for (const free_point of t.neighbors.empties) {
  ...
  for (const [dx, dy] of [
    ...
    switch (tile_type) {
      ...
      case 'O':
        ...
        break;
      case '.':
        neighbors.empties.push(neighbor_tile);
        break;
      case '#':
...
const getExpansionMove = (ns: NS, tiles: Tile[][]) => {
  ...
  moveOptions = moveOptions.sort((m1, m2) => m2.neighbors.empties.length - m1.neighbors.empties.length);
  moveOptions = moveOptions.filter((m) => m.neighbors.empties == moveOptions[0].neighbors.empties);

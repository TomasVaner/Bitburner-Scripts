...
export async function main(ns: NS) {
  const operation = "weaken";
  const flag = ns.flags(flags_data) as typeof flags_struct;

  let logger = new Logger(ns, { log_file: flag.log_file, clean: false, extra_name: flag.target, create_file: false });
  ...
  const result = await ns.weaken(flag.target, { additionalMsec: flag.delay });
  ns.atExit(() => {
    if (flag.port_index != -1)
    {
      let return_data = new HackOperationResultPacket(flag.target, result, operation, ns.pid);

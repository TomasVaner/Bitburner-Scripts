...
export async function main(ns: NS) {
  ...
  if (details.modelId.length == 0)
    return;
  let model_id = details.modelId.replaceAll(' ', '_');

  let logger = new Logger(ns, {extra_name:`${model_id.replaceAll(' ', '_')}/${hostname}`})

  let res = await authenticate(hostname);
  details = ns.dnet.getServerAuthDetails(hostname);
  if (!res.success)
  {
    if (details.isConnectedToCurrentServer && details.isOnline)
    { 
      logger.Log(`WARNING: ${ns.getHostname()} -> ${hostname}: ${details.modelId} ${JSON.stringify(details)} CHA:${ns.dnet.getServerRequiredCharismaLevel(hostname)} ${res.source} log provided: ${ns.fileExists(logger.log_file)} (${logger.log_file}). Initial model_id: ${model_id}`, {global_log:true});      
      if (ns.fileExists(logger.log_file))
        ns.scp(logger.log_file, "home");
  ...
  else
  {
    if (res.pass === undefined)
      throw `${ns.getHostname()} -> ${hostname}: ${details.modelId} ${JSON.stringify(details)} CHA:${ns.dnet.getServerRequiredCharismaLevel(hostname)} ${res.source} log provided: ${ns.fileExists(logger.log_file)} (${logger.log_file}). Initial model_id: ${model_id}`;
    let pass_data = new NewPasswordPacket(hostname, details.modelId, res.pass, res.source);
  ...
  switch (details.modelId) {
    ...
    default:
      if (details.isOnline && details.isConnectedToCurrentServer && details.modelId.length > 0)
        logger.Log(`ERROR: ${hostname}: Unrecognized modelId: ${details.modelId}, ${JSON.stringify(details)}`, {global_log:true});
...
{
    let ret = [] as any[];
    if (!IsServerReachable(hostname))
      return ret;
    if (ns.dnet.getServerRequiredCharismaLevel(hostname) > ns.getPlayer().skills.charisma)
        return ret;

    let result = await ns.dnet.heartbleed(hostname, {peek:true, logsToCapture:200});
    if (!result.success)
        return ret;
    for (let log of result.logs)
    {
        let test_res = SafeJSONParse(log);
        if (test_res === null)
        {
            continue;
        }
...
async function AuthenticateWithMemo (hostname:string) {
  const details = ns.dnet.getServerAuthDetails(hostname);
  let pass = details.passwordHint.slice(-details.passwordLength);
...
async function AuthenticateWithPr0verFl0 (hostname:string) {
  const details = ns.dnet.getServerAuthDetails(hostname);
  let pass = 'a'.repeat(details.passwordLength*2);
...
async function AuthenticateWithBellaCuore (hostname:string) {
  ...
  {
    let value = roman_numerals.get(details.data[i]) ?? 0
    let value_next = i < details.data.length - 1 ? (roman_numerals.get(details.data[i+1]) ?? 0) : 0;
...
async function AuthenticateWithCloudBlare (hostname:string) {
  ...
  for (let i = 0; i < details.data.length; ++i)
    if (details.data[i] >= '0' && details.data[i] <= '9')
      pass += details.data[i];
  const result = await ns.dnet.authenticate(hostname, pass);
  // TODO: store discovered passwords somewhere safe, in case we need them later
  return {pass:pass, success:result.success, source: "authenticate"};
...
async function AuthenticateWithOctantVoxel (hostname:string) {
  const details = ns.dnet.getServerAuthDetails(hostname);
  let [base_str, num_str] = details.data.split(',');
  let base = Number(base_str);
  let num = parseInt(num_str, base);
...
async function AuthenticateWithFreshInstall (hostname:string) {
  ...
  let passwords = ['admin', 'password', '0000', '12345'];

  for (let pass of passwords.filter(p => p.length == details.passwordLength))
  {
    const result = await ns.dnet.authenticate(hostname, pass);
    if (result.success)
...
async function AuthenticateWithLaika4 (hostname:string) {
  ...
  let passwords = ['fido', 'spot', 'rover', 'max'];
...
async function AuthenticateWithNIL (hostname:string) {
  ...
  let yes = "_".repeat(details.passwordLength).split('');
  let no = [] as string[][];
  ...
  {
    if (!IsServerReachable(hostname))
      return {success: false, source: "Server unreachable"};

    let logs = await Heartbleed(hostname);
    for (let log of logs)
    {
      logger.Log(JSON.stringify(log));
      if (log.code == 200)
      {
        let result = await ns.dnet.authenticate(hostname, log.passwordAttempted);
        if (result.success)
        {
      ...
      {
        let y = log.data.split(',');
        for (let ind = 0; ind < y.length; ++ind)
        {
          if (y[ind] == 'yes')
            yes[ind] = log.passwordAttempted[ind];
          else if (!no[ind]?.includes(log.passwordAttempted[ind]))
            no[ind]?.push(log.passwordAttempted[ind]);
        }
      }
    }

    details = ns.dnet.getServerAuthDetails(hostname);
    if (details.passwordLength != yes.length)
      return {success:false, source:"Password length mismatch"};

    let pass_next = '_'.repeat(details.passwordLength).split('');

    for (let ind = 0; ind < details.passwordLength; ++ind)
    {
      if (yes[ind] != '_')
        pass_next[ind] = yes[ind]
      else
      {
        for (let l of alphabet)
        {
          if (no[ind].includes(l))
            continue;
          pass_next[ind] = l;
          break;
        }
      ...
      if (pass_next[ind] == '_')
        return {success:false, source:"Could not find the next symbol"};
    }

    if (pass == pass_next.join(''))
      return {success: false, source: `Password cycle (${pass})`};
    pass = pass_next.join('');
    logger.Log(`Trying password ${pass}`);
    let result = await ns.dnet.authenticate(hostname, pass);
    if (result.success)
    {
      return {pass:pass, success:result.success, source: "authenticate"};
    }
...
async function AuthenticateWithAccountsManager (hostname:string) {
  ...
  while (from <= to)
  {
    ...
    {
      logger.Log(JSON.stringify(log))
      if (log.passwordAttempted.length != details.passwordLength)
        continue;
      if (log.code == 200)
      {
        let result = await ns.dnet.authenticate(hostname, log.passwordAttempted);
      ...
      {
        let guess = Number(log.passwordAttempted)
        if (Number.isNaN(guess))
          continue;
        if (log.data == "Lower")
        {
          if (guess - 1 < to)
            logger.Log(`Reducing to: ${to} -> ${guess - 1}`)
          to = Math.min(to, guess - 1);
        }
        else if (log.data == "Higher")
        {
          if (guess + 1 > to)
            logger.Log(`Increasing from: ${from} -> ${guess + 1}`)
    ...
    let guess = Math.round((from + to)/2);
    let pass = guess.toString().padStart(details.passwordLength);
    logger.Log(`Trying password ${pass}`);
    let result = await ns.dnet.authenticate(hostname, pass);
    if (result.success)
      return {pass:pass, success:true, source: "authenticate"};
...
async function AuthenticateWithFactorio (hostname:string) {
  ...
  for (let num = 0; num < 10 ** details.passwordLength; ++num)
  {
    ...
    for (let log of logs)
    {
      ...
      if (log.code == 200)
      {
        ...
        if (result.success)
        {
          ...
          logger.Log(`Password ${log.passwordAttempted} failed even though was got from heartbleed`);
      }
      else
      {
        let log_num = Number(log.passwordAttempted);
        if (num == log_num)
          num = log_num + 1;
    ...
    if (num_before != num)
      logger.Log(`Skipped num: ${num_before} -> ${num}`);
...
async function AuthenticateWithPHP (hostname:string) {
  const details = ns.dnet.getServerAuthDetails(hostname);
  let data = details.data.split("");
  let identity = [] as number[];
  let perm = [] as number[];
  ...
  do {
    ...
    let t = perm[index_swap];
    perm[index_swap] = perm[index_swap - 1];
    perm[index_swap - 1] = t;
    --index_swap;
    if (index_swap == 0)
      index_swap = details.passwordLength - 1;
    counter++;
    if (counter > details.passwordLength ** details.passwordLength)
      break;
  ...
  for (let num = 0; num < 10 ** details.passwordLength; ++num)
  {
    ...
    for (let log of logs)
    {
      ...
      else
      {
        ...
        let [g, r] = log.data.split(',');
        let [green, red] = [Number(g), Number(r)];

        let pass_array = log.passwordAttempted.split('');

        if (green + red == 0)
        {
          for (let d of pass_array)
          {
  ...
  for (let t = 0; t < 2; ++t)
  {
    ...
    for (let log of logs)
    {
      ...
      else
      {
        ...
        let regex = new RegExp(`${escaped_host}:(\\d{${details.passwordLength}})`);
        logger.Log(`Looking for a match to '${regex.source}'`)
        let match = log.data.match(regex);
        if (match)
        {
          pass = match[1];
        }

...
export async function main(ns:NS) {
  ...
  let [controller_pid] = ns.args as [number];
  ns.clearPort(ns.pid);

  let reported_contracts = [] as string[];
  ...
  while (true) {
    ...
    for (let file of ns.ls(ns.getHostname()))
    {
      ...
      if (file.endsWith('.cct') && !reported_contracts.includes(file))
      {
        ...
        let report_obj = new DarknetContractReportPacket(ns.getHostname(), file);
    ...
    for (const hostname of nearbyServers) {
      ...
      if (!authenticationResult.success)
      {
        if (all_neighbours_connected)
          logger.Log(`${hostname} is still disconnected`);
      ...
      if (!ns.isRunning(ScriptNames.prober, hostname, ...ns.args))
      {
        ...
        ns.exec(ScriptNames.prober, hostname, {
    ...
    if (ns.ls(ns.getHostname(), ".cache").length > 0
      && !ns.isRunning(ScriptNames.open_chache))
    {
...
async function ServerSolver (hostname:string) {
  ...
  let pw_req_data = new RequestPasswordPacket(hostname, details.modelId, ns.pid);
  ...
  let return_data_json = await readPortInfo(ns.pid);

  logger.Log(`${ns.pid}: got data ${return_data_json}`);
  if (return_data_json && return_data_json != Strings.null_port_data)
  {
    let return_data = JSON.parse(return_data_json) as RequestPasswordResponse;
...
async function readPortInfo(port: number) {
  if (ns.peek(port) !== Strings.null_port_data) 

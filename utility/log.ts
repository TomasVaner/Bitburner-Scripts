export class Logger {
  constructor(ns: NS, { log_file = '', clean = true, extra_name = '', create_file = true } = {}) {
    ...
    if (this.log_file)
      ns.atExit(() => {
        ns.print(`Log file: ${this.log_file}`);
      }, 'log_file');
  }
  ...
  Log(message: string, { file = true, global_log = false, include_timestamp = undefined as boolean | undefined } = {}) {
    if (!this.logger_enabled) return;
    if (include_timestamp === undefined) include_timestamp = this.include_timestamp;
    const ts = include_timestamp ? `[${performance.now().toFixed(1)}] ` : '';
    this.ns.print(`${this.prefix}${ts}${message}`);
    if (global_log) this.ns.tprint(`${this.prefix}${ts}${message}`);
    if (file && this.log_file) this.ns.write(this.log_file, `${this.prefix}${ts}${message}\n`, 'a');
  }
}
